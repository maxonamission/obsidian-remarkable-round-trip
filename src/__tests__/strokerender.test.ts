...
describe("planRender", () => {
	...
	it("crops to the ink so handwriting is not lost on a mostly empty page", () => {
		const plan = planRender([stroke([{ x: 100, y: 200 }, { x: 140, y: 220 }])], {
			padding: 10,
			maxSize: 1000,
		});
	...
	it("renders the whole page when cropping is off, scaled to fit maxSize", () => {
		const plan = planRender([stroke([{ x: 0, y: 0 }, { x: 10, y: 10 }])], {
			crop: false,
			maxSize: 1404,
		});
	...
	it("keeps the image within the requested maximum size", () => {
		const plan = planRender([stroke([{ x: -700, y: 0 }, { x: 700, y: 1800 }])], {
			maxSize: 800,
		});
