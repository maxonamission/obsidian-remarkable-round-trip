...
/** Collect the highlights of one device document, ordered by page. */
export async function collectHighlights(
	deviceDocId: string,
	hash: string,
	deps: PullDeps,
): Promise<Highlight[]> {
	const files = (await deps.listDocumentFiles(deviceDocId, hash)).filter((file) =>
		isHighlightFile(file.id),
	);
	...
	return perFile.flatMap((entry) => entry.highlights);
}
	...
	for (const entry of entries) {
		...
		try {
			if (hash === undefined) {
				...
				const highlights = await collectHighlights(entry.deviceDocId, hash, deps);
